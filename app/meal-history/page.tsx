...
export default function MealHistoryPage() {
  ...
  const viewMealDetails = (meal: MealHistoryEntry) => {
    ...
    // Navigate to meal analysis page with the parameters
              ...
              {Object.entries(groupedMeals).map(([date, dateMeals]) => (
                <div key={date} className="bg-white rounded-lg shadow-md overflow-hidden">
                  ...
                  <div className="divide-y">
                    {dateMeals.map(meal => (
                      <div key={String(meal.id)} className="p-4 hover:bg-gray-50 transition-colors">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="mt-2 flex space-x-3">
                              ...
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleDelete(String(meal.id));
                                }}
                                className="text-red-600 hover:text-red-800 text-sm font-medium"
                                disabled={deletingMealId === String(meal.id)}
                              >
                                {deletingMealId === String(meal.id) ? "Deleting..." : "Delete"}
                              </button>
                            </div>
                          </div>
                        </div>
